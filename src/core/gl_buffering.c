...
static void load_polygon_into_data(t_software_environ *env, t_polygon *polygon, void *buffer)
{
	...
	while (x)
	{
		...
		memcpy((uint8_t *) buffer + i * vertex_size + sizeof(t_vec3) + sizeof(t_vec3),
		       vertex->normal, sizeof(t_vec3));

		uv = vertex->uv == NULL ? &uvs[i % 3] : vertex->uv;

		memcpy((uint8_t *) buffer + i * vertex_size + sizeof(t_vec3) + sizeof(t_vec3) + sizeof(t_vec3), uv, sizeof(t_vec2));
