...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/08/26 19:02:35 by aemilien          #+#    #+#             */
/*   Updated: 2019/08/26 19:02:37 by aemilien         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		int mods)
{
	...
	(void)mods;
	i = 0;
	while (sizeof(t_keyboard_callback_map) * i
			< sizeof(g_keyboard_callback_map))
	{
		if (g_keyboard_callback_map[i].key == key
				&& g_keyboard_callback_map[i].action == action)
		{
			g_keyboard_callback_map[i].f(g_env, window);
			break ;
		}
		i++;
	}
