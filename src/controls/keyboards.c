...
void switch_render_style(t_software_environ *env, GLFWwindow *window)
{
	...
	while (sizeof(GLenum) * i < sizeof(g_render_styles))
	{
		...
		{
			env->render_style = sizeof(GLenum) * (i + 1) < sizeof(g_render_styles) ? g_render_styles[i + 1] : g_render_styles[0];
			return;
		}
...
void render_greyscale(t_software_environ *env, GLFWwindow *window)
{
	(void) window;
...
void move_forward_on_axis(t_software_environ *env, GLFWwindow *window)
{
	(void) window;
...
void move_backward_on_axis(t_software_environ *env, GLFWwindow *window)
{
	(void) window;
